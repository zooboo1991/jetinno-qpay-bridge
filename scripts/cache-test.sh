#!/usr/bin/env bash
# Runs scripts/cache-test.mjs against a throwaway Postgres carrying the real
# migrations. Never touches Supabase; needs no production credential.
set -uo pipefail
cd ~/Downloads/jetinno-qpay-bridge

PORT=55447
NAME=jetinno-cache-test
...
DATABASE_URL="postgresql://postgres:test@localhost:$PORT/bridge" node scripts/cache-test.mjs
